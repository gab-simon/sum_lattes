...
int getPeriodicsByYear(researcherRoot_t *researchers, int classification, int year) {
    ...
    while(periodicAux) {
        if(periodicAux->year == year && periodicAux->classificationInt == classification)
            count++;
...
int getEventsByYear(researcherRoot_t *researchers, int classification, int year) {
    ...
    while(eventAux) {
        if(eventAux->year == year && eventAux->classificationInt == classification)
            count++;
...
void printByYear(researcherRoot_t *researchers, int yearDelimiter) {
    ...
    if(yearDelimiter != 0)
        min = yearDelimiter;
    int i, j;
    char classifications[10][3] = {"NC","A1", "A2", "A3", "A4", "B1", "B2", "B3", "B4", "C"};
